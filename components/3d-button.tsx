...
const buttonVariants = cva(
  'inline-flex items-center justify-center whitespace-nowrap rounded-3xl text-sm font-semibold ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 gap-2 border',
  {
    variants: {
      variant: {
        ai: 'bg-indigo-500 text-white hover:bg-indigo-600 border-indigo-700 border-b-4 border-b-indigo-600 shadow-md',
        default:
          'bg-blue-500 text-primary-foreground hover:bg-blue-600 border-blue-700 border-t-2 border-b-blue-600 shadow-md',
        destructive:
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    return (
      ...
      >
        {isLoading ? (
          <IconLoader2 size={TABLER_ICON_SIZE} className="animate-spin" />
        ) : (
          <></>
        )}
        {!isLoading && supportIcon && (
          <SupportIconRender size={TABLER_ICON_SIZE} />
        )}
        {children}
